...
var allRecords=function(ename){
	base.logger4js_api.trace("[allRecords(%o)]", ename);
	var sqlStr = "";
	...
	base.logger4js_api.trace("[allRecords(%o)]%d records found", ename, data_ret.length);
	return data_ret;
};

// 在表名为ename的表中根据guid查找一条记录，返回：{guid:'...',...}或null
var recordByGuid=function(ename, guid){
	base.logger4js_api.trace("[recordByGuid(%o,%o)]", ename, guid);
	var sqlStr = "";
	...
	if(data_ret.length == 0){
		base.logger4js_api.trace("[recordByGuid(%o,%o)]record not found", ename, guid);
		return null;
	}else{
		base.logger4js_api.trace("[recordByGuid(%o,%o)]record found", ename, guid);
		return data_ret[0];
	}
};
...
var delRecords=function(ename, guidArray){
	base.logger4js_api.trace("[delRecords(%o,%o)]", ename, guidArray);
	var sqlStr = "";
	var ret = [];
...
var updateRecords=function(ename, records){
	...
	var data_ret = dbOpr.exec_sync(fileName_data, sqlStr);
	if ('error' in data_ret){throw data_ret;}
	base.logger4js_api.trace("[updateRecords(%o,%o)]%d records need update:%o", ename, records.length, data_ret.length, data_ret);

	for(var i in records){
		if(data_ret.findIndex(function(v){return records[i]['guid']==v['guid'];}) >= 0){
			// 更新记录
	...
	base.logger4js_api.trace("[updateRecords(%o,%o)]success", ename, records.length);
}

// 在表名为ename的表中根据guid更新一条记录（不检测guid是否存在）
var updateRecord=function(ename, record){
	base.logger4js_api.trace("[updateRecord(%o,%o)]", ename, record['guid']);
	var sqlStr = "";
	...
	base.logger4js_api.trace("[updateRecord(%o,%o)]succ", ename, record['guid']);
}

// 在表名为ename的表中插入新记录（不检测guid是否重复）
var insertRecord=function(ename, record){
	base.logger4js_api.trace("[insertRecord(%o,%o)]", ename, record['guid']);
	var guid = record['guid'];
	if(guid == null){
		guid = base.newGuid();
		base.logger4js_api.trace("[insertRecord(%o)generate guid:%s]", ename, guid);
	}
	var sqlStr = "";
	sqlStr = "INSERT INTO TBL_DATA_" + ename + " (guid,";
	for(var i in record){
		sqlStr += i +",";
	};
	sqlStr = sqlStr.substr(0, sqlStr.length-1);
	...
	base.logger4js_api.trace("[insertRecord(%o,%o)]succ", ename, guid);
}
