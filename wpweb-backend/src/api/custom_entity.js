...
var allCompleteEntityMetaData=function () {
	base.logger4js_api.trace("[allCompleteEntityMetaData()]");
	var ret = [];
	
	var names = allEntityName();	
	base.logger4js_api.trace("[allCompleteEntityMetaData()]%d entity names found", names.length);
	for(var i in names){
	...
	base.logger4js_api.trace("[allCompleteEntityMetaData()]%d entity found success", ret.length);
	return ret;
}

// 返回所有实体名(name)，返回数组
var allEntityName=function(){
	base.logger4js_api.trace("[allEntityName()]");
	var sqlStr = "";
	var ret = [];
	...
	base.logger4js_api.trace("[allEntityName()]%d entity found", data_ret.length);
	...
	base.logger4js_api.trace("[allEntityName()]%d entity names success", ret.length);
	return ret;
}

// 根据实体name字段获取该实体的完整元数据（包括字段定义），不存在则返回null
var completeEntityMetaData=function(name){
	base.logger4js_api.trace("[completeEntityMetaData(%s)]",name);
	var sqlStr = "";
	var ret = {};
	...
	ret['name']= name;
	base.logger4js_api.trace("[completeEntityMetaData(%s)]data table found",name);
	...
	base.logger4js_api.trace("[completeEntityMetaData(%s)]meta data found",name);

	// 查找该表的所有字段
	sqlStr = "PRAGMA TABLE_INFO('TBL_DATA_" + name + "')";
    data_ret = dbOpr.exec_sync(fileName_data, sqlStr);
	if ('error' in data_ret){throw data_ret;}
	base.logger4js_api.trace("[completeEntityMetaData(%s)]data table %d columns found",name, data_ret.length);
	
	// 在元数据表中查找字段的定义
	sqlStr = "SELECT eName,name,label,title FROM TBL_meta_EntityFieldDefine WHERE eName='" + name + "'";
    meta_ret = dbOpr.exec_sync(fileName_meta, sqlStr);
	if ('error' in meta_ret){throw meta_ret;}
	base.logger4js_api.trace("[completeEntityMetaData(%s)]meta data %d fields found",name, meta_ret.length);
	...
	ret['fields']= fields;
	base.logger4js_api.trace("[completeEntityMetaData(%s)]found success",name);
	return ret;
}

// 更新一批实体定义，参数是数组，对于其中每一个元素，将检查实体定义是否存在，若不存在则新建，否则只更新
var updateEntitys=function (entitys) {
	base.logger4js_api.trace("[updateEntitys(%d)]",entitys.length);
	if((entitys instanceof Array)==false) throw new Error('format error');
	if(entitys.length==0) throw new Error('format error');
	
	var names = allEntityName();
	base.logger4js_api.trace("[updateEntitys(%d)]entity names in db: ",entitys.length, names);
	entitys.forEach(function(item, index, arr){
		base.logger4js_api.trace("[updateEntitys(%d)]insert or update entity %s",entitys.length, item['name']);
...
var newEntity=function (entity) {
	base.logger4js_api.trace("[newEntity(%s)]", entity['name']);
	var sqlStr = '';
	var ret = null;
	...
	base.logger4js_api.trace("[newEntity(%s)]create data table success", entity['name']);
	...
	base.logger4js_api.trace("[newEntity(%s)]create meta data success", entity['name']);
	...
	base.logger4js_api.trace("[newEntity(%s)]create %d fields meta data success", entity['name'], insertFields.length);
}

// 更新一个实体定义，实体定义本身仅需更新元数据信息，字段信息的变更需要更改数据表
// 不支持字段改名，不支持字段删除，不支持字段类型修改
var updateEntity=function(entity) {
	base.logger4js_api.trace("[updateEntity(%s)]", entity['name']);
	var sqlStr = '';
	var ret = null;
	...
	base.logger4js_api.trace("[updateEntity(%s)]old defination found", entity['name']);
	...
	if((oldEntity['label']!=entity['label'])||(oldEntity['title']!=entity['title'])){
		...
		base.logger4js_api.trace("[updateEntity(%s)]update meta data success", entity['name']);
	}else{
		base.logger4js_api.trace("[updateEntity(%s)]entity(without fields) meta data same, do not update", entity['name']);
	}
	...
	if(true){//... 字段是否需要修改未实现
		...
		base.logger4js_api.trace("[updateEntity(%s)]delete column meta data success", entity['name']);
		var insertFields=[];
		for(var i in entity['fields']){
			insertFields.push({
				eName:entity['name'],
		...
		base.logger4js_api.trace("[updateEntity(%s)]create %d fields meta data success", entity['name'], insertFields.length);
		...
		for(var i in entity['fields']){
			...
			if(oldIndex==-1){
				...
				base.logger4js_api.trace("[updateEntity(%s)]add column %s success", entity['name'], entity['fields'][i]['name']);
			}else{
				var sqlArray = [];
				...
				if(oldEntity['fields'][oldIndex]['type']!= entity['fields'][i]['type']){
					...
					base.logger4js_api.trace("[updateEntity(%s)]modify column %s type success", entity['name'], entity['fields'][i]['name']);
				}else{
					base.logger4js_api.trace("[updateEntity(%s)]column %s type do not need modify", entity['name'], entity['fields'][i]['name']);
				}
			}
		}
		for(var i in oldEntity['fields']){
			if(entity['fields'].findIndex(function(v){return oldEntity['fields'][i]['name']==v['name'];})==-1){
				...
				base.logger4js_api.trace("[updateEntity(%s)]delete column %s success", entity['name'], entity['fields'][i]['name']);
			}
		}
		base.logger4js_api.trace("[updateEntity(%s)]modify %d column data success", entity['name'], entity['fields'].length);
	}
	else{
		base.logger4js_api.trace("[updateEntity(%s)]fields configure do not need modify", entity['name']);
	}
}

// 根据名称删除一批实体定义，参数：name1,name2,...
var delEntitys=function(names){
	base.logger4js_api.trace("[delEntitys(%s)]", names);

	var nameArray=names.split(',');
	for(var i in nameArray){
		delEntity(nameArray[i]);
	}
	base.logger4js_api.trace("[delEntitys(%s)]OK", names);
}

// 根据名称删除实体定义，顺序：1、删除数据；2、删除字段定义；3、删除实体定义
var delEntity=function(name){
	base.logger4js_api.trace("[delEntity(%s)]", name);
	var ret = null;
	
	// 删除数据表
	var sqlStr="DROP TABLE IF EXISTS TBL_DATA_" + name;
	ret = dbOpr.exec_sync(fileName_data, sqlStr);
	if(ret != 0){
        throw ret;
    }
	base.logger4js_api.trace("[delEntity(%s)]delete data table success", name);
	
	// 删除元数据定义
	var sqlStr1="DELETE FROM TBL_meta_EntityFieldDefine WHERE eName='" + name + "'";
	var sqlStr2="DELETE FROM TBL_meta_EntityDefine WHERE name='" + name + "'";
	ret = dbOpr.execBatch_sync(fileName_meta, [sqlStr1, sqlStr2]);
	if('error' in ret){
        throw ret;
    }
	base.logger4js_api.trace("[delEntity(%s)]delete meta data success", name);
}
